...
import com.jb.domain.integration.iron.IronException
...
object upickle:
  ...
  /** A `Reader` for refined types using uPickle. Decodes to the underlying type then
    * checks the constraint. Throws custom `IronException` in case of failure to be
    * later handled by Codec` to return `DecodingResult.InvalidValue` instead of
    * `DecodeResult.Error`
    *
    * @param reader
  ...
  /** A `Writer` instance for refined types using uPickle. This is essentially the
    * underlying type `Writer`.
    *
    * @param writer
    *   the `Writer` of the underlying type.
    */
  inline given [A, B](using inline writer: Writer[A]): Writer[A :| B] =
    writer.asInstanceOf[Writer[A :| B]]
